...
class FlowSolver(object):
    ...
    def __init__(self, flow_params, geometry_params, solver_params):
        ...
        bcu_wall = DirichletBC(V, Constant((u_wall, v_wall)), surfaces, wall_tag)
        ...
        jets = [Expression(('0', '-(3/2)*(Q/width)*(1-((2*x[0]-length_cylinder+width)/width)^2)'),  # top jet
                           width=width, length_cylinder=length_cylinder, Q=0, degree=1),
                Expression(('0', '(3/2)*(Q/width)*(1-((2*x[0]-length_cylinder+width)/width)^2)'),  # bot jet
                           width=width, length_cylinder=length_cylinder, Q=0, degree=1)]
